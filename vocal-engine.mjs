...
function chooseAttempt(performanceMode='auto'){
  ...
  if(performanceMode==='wasm')return[{dtype:'q8',device:'wasm'}];
  if(performanceMode==='webgpu')return canGpu?[{dtype:'q8',device:'webgpu'},{dtype:'q8',device:'wasm'}]:[{dtype:'q8',device:'wasm'}];
  return canGpu&&!save?[{dtype:'q8',device:'webgpu'},{dtype:'q8',device:'wasm'}]:[{dtype:'q8',device:'wasm'}];
}
...
async function renderNeuralGenderStem(instrumental,records,gender,voice,level,tts,onStatus){const relevant=records.filter(r=>r.singers.includes(gender));if(!relevant.length)return null;const sr=instrumental.sampleRate,ctx=new OfflineAudioContext(2,instrumental.length,sr),bus=setupVocalBus(ctx,gender);bus.out.connect(ctx.destination);const slot=Math.max(1.45,(instrumental.duration-4)/Math.max(1,records.length));let done=0;for(const record of relevant){const key=`${voice}|${record.text}`;let buf=lineCache.get(key);if(!buf){onStatus?.(`Generating ${gender} voice ${++done}/${relevant.length}…`);let raw;try{raw=await tts.generate(record.text,{voice,speed:.98});}catch(err){const fallbackVoice=gender==='male'?'am_michael':'af_heart';if(voice===fallbackVoice)throw err;onStatus?.(`Retrying that line with the standard ${gender} voice…`);raw=await tts.generate(record.text,{voice:fallbackVoice,speed:.98});}buf=await decodeBlob(rawAudioToBlob(raw));cachePut(key,buf);await Promise.resolve();}const start=2+record.index*slot;scheduleSpeechLine(ctx,bus.input,buf,start,slot,record.section,record.index,level);}onStatus?.(`Rendering ${gender} vocal stem…`);return await ctx.startRendering();}
...
export async function renderVocalMix({instrumentalBuffer,lyrics,arrangement='male',maleVoice='am_michael',femaleVoice='af_heart',level=.9,mode='neural',performanceMode='auto',chorusTogether=true,onStatus}){
  ...
  if(mode==='neural'){
    try{const tts=await loadModel(onStatus,performanceMode);male=await renderNeuralGenderStem(instrumentalBuffer,records,'male',maleVoice,level,tts,onStatus);female=await renderNeuralGenderStem(instrumentalBuffer,records,'female',femaleVoice,level,tts,onStatus);}
